var vm = new Vue({
    ...
    watch: {
        ...
        'form_address.province_id': function () {
            if (this.form_address.province_id) {
                var url =  '/areas/?area_id=' + this.form_address.province_id;
                axios.get(url, {
                    responseType: 'json'
                })
                    .then(response => {
        ...
        'form_address.city_id': function () {
            if (this.form_address.city_id) {
                var url =  '/areas/?area_id=' + this.form_address.city_id;
                axios.get(url, {
                    responseType: 'json'
                })
                    .then(response => {
    ...
    methods: {
        ...
        save_address(){
            ...
            if (this.error_receiver || this.error_place || this.error_mobile || this.error_email || !this.form_address.province_id || !this.form_address.city_id || !this.form_address.district_id) {
                ...
                if (this.editing_address_index === '') {
                    ...
                    // 修改地址
                    var url = '/addresses/' + this.addresses[this.editing_address_index].id + '/';
        ...
        delete_address(index){
            var url = '/addresses/' + this.addresses[index].id + '/';
            axios.delete(url, {
                headers: {
                    'X-CSRFToken': getCookie('csrftoken')
                },
        ...
        // 设置默认地址
        set_default(index){
            var url = '/addresses/' + this.addresses[index].id + '/default/';
            axios.put(url, {}, {
                headers: {
                    'X-CSRFToken': getCookie('csrftoken')
                },
        ...
        save_title(index){
            if (!this.input_title) {
                ...
                var url = '/addresses/' + this.addresses[index].id + '/title/';
                axios.put(url, {
